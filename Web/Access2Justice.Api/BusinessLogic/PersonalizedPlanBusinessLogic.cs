...
namespace Access2Justice.Api.BusinessLogic
{
	public class PersonalizedPlanBusinessLogic : IPersonalizedPlanBusinessLogic
	{
		...
		public async Task<Document> UpsertPersonalizedPlanAsync(PersonalizedPlanViewModel personalizedPlan, string oId)
		{
			try
			{
				...
				if (userPersonalizedPlan == null || userPersonalizedPlan?.PersonalizedPlanId == Guid.Empty)
				{
					...
					await UpdatePlanToProfile(personalizedPlan.PersonalizedPlanId, oId);
					return newPlan;
				}
				else
				{
		...
		public async Task UpdatePlanToProfile(Guid planId, string oId)
		{
