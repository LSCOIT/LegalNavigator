    ...
    public class UserRoleBusinessLogic : IUserRoleBusinessLogic
    {
        ...
        public async Task<List<string>> GetPermissionDataAsync(string oId)
        {
            ...
            if (userProfile?.RoleInformationId.Count() > 0)
            {
                ...
                var permissionData = await dbClient.FindItemsWhereInClauseAsync(dbSettings.UserRolesCollectionId, Constants.Id, roleIdsList);
        ...
        {
            var roleData = await dbClient.FindItemsWhereInClauseAsync(dbSettings.UserRolesCollectionId, Constants.Id, roleInformationId);
