...
export class SubtopicsComponent implements OnInit {
  ...
  guidedInput: ISubtopicGuidedInput = {
    activeId: "",
    name: "",
    guidedAssistantId: "",
  };
  ...
  getSubtopics(): void {
    ...
    this.topicService.getDocumentData(this.activeTopic).subscribe((topic) => {
      ...
      if (topic[0]) {
        this.icon =
          topic[0].icon == ""
            ...
            : topic[0].icon;
        this.guidedInput = {
          activeId: this.activeTopic,
          name: this.topic.name,
          guidedAssistantId: this.topic.curatedExperienceId,
        };
      ...
      this.topicService
        .getSubtopics(this.activeTopic)
        .subscribe((subtopics) => {
          this.subtopics = subtopics.sort((n1, n2) => {
          ...
          if (this.subtopics.length === 0) {
            this.topicService
              .getSubtopicDetail(this.activeTopic, "")
              .subscribe((data) => {
                if (data.length > 0) {
