...
describe("Service:PersonalizedPlan", () => {
  ...
  let mockProfileSavedResources = {
    ...
    topics: [
      {
        icon:
          "https://cs4892808efec24x447cx944.blob.core.windows.net/static-resource/assets/images/categories/money_debt.svg",
        id: "5970328f-ceea-4903-b4dc-1ea559623582",
        resourceType: "Topics"
      }
    ]
  };
  ...
  let mockResourceInput = {
    ...
    Location: null,
    PageNumber: 0,
    ResourceIds: ["afbb8dc1-f721-485f-ab92-1bab60137e24"],
    ResourceType: "ALL",
  ...
  beforeEach(() => {
    ...
    service = new PersonalizedPlanService(
      httpSpy,
      arrayUtilityService,
      toastrService,
      global,
  ...
  it("should user Saved Resources in getPersonalizedResources method", done => {
    ...
    service
      .getPersonalizedResources(mockResourceInput)
      .subscribe(userResources => {
        expect(httpSpy.post).toHaveBeenCalled();
  ...
  it("should get topics details in getTopicDetails method of service", done => {
    ...
    mockPersonalizedPlanService
      .getTopicDetails(mockIntentInput)
      .subscribe(topics => {
        expect(httpSpy.post).toHaveBeenCalled();
  ...
  it("should call getTopicsFromGuidedAssistant with session stored resources in saveResourcesToUserProfile method", () => {
    spyOn(sessionStorage, "getItem").and.returnValue(
      JSON.stringify(mockSavedResources)
    );
  ...
  it("should return resource ids from resources list", () => {
    let mockResources = [
      { id: "resource1", resourceType: "Organizations" },
      { id: "resource2", resourceType: "Articles" }
    ];
